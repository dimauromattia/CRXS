...
void CRACS::ConfigHandler::SetInput(int argc, char *argv[], std::string description){
    ...
    std::string mySoft = "/home/mk793919/software/master";
    if (const char* env_p = std::getenv("PROPERFIT")) {
        mySoft = env_p;
    }else{
        std::cout << "Error: environment variable PROPERFIT not set." << std::endl;
    }

    fStandardGaldefFile = mySoft+"/data/galprop/galdef_54_standard";
    AddOption        (  "standardGaldef",    fStandardGaldefFile,     "Full path of standard galdef file. Default: $PROPERFIT/data/galprop/galdef_54_standard"     );

    fSoftwarePath  = mySoft;
    AddOption        (  "softwarePath",      fSoftwarePath,           "Full path of of the software package location. Default: $PROPERFIT"                         );
};
